...
def filter_tasks(obs: TabularRepresentation[ArrayLike], status: Status) -> TabularRepresentation[ArrayLike]:
    """
    Filters the tasks in the observation based on their status.
    """
    if isinstance(obs, dict):
        return {k: v for k, v in obs.items() if k != status and isinstance(v, ArrayLike)}
...
class PriorityDispatchingRule(ABC):
    ...
    def __call__(self, obs: Any, time: int | None = None) -> Sequence[SingleAction]:
        filtered_obs = filter_tasks(
            wrap_observation(obs), self.status
        )
        ...
        action = [
            (self.instruction, int(task_id)) for task_id in order
        ]
