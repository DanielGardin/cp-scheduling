...
# from cpscheduler.environment._protocols import ArrayLike, TabularRepresentation
...
#     def _to_actions_1d(self, priorities: ArrayLike) -> list[SingleAction]:
#         """
#         Convert a 1D priority array to an action list, skipping masked tasks.
#         """
#         if TORCH_AVAILABLE and isinstance(priorities, torch.Tensor):
#             valid_ids = torch.nonzero(
#                 ~torch.isneginf(priorities), as_tuple=False
#             ).flatten()
#             if valid_ids.numel() == 0:
#                 return []

#             local_order = torch.argsort(
#                 priorities[valid_ids], descending=True, stable=True
#             )
#             ordered_ids = valid_ids[local_order].tolist()
#             return [(self.instruction, int(task_id)) for task_id in ordered_ids]

#         if NUMPY_AVAILABLE and isinstance(priorities, np.ndarray):
#             valid_ids = np.flatnonzero(~np.isneginf(priorities))
#             if valid_ids.size == 0:
#                 return []

#             local_order = np.argsort(-priorities[valid_ids], kind="stable")
#             ordered_ids = valid_ids[local_order]
#             return [(self.instruction, int(task_id)) for task_id in ordered_ids]
...
#         ]
#         valid_ids.sort(key=lambda i: priorities[i], reverse=True)

#         return [(self.instruction, int(task_id)) for task_id in valid_ids]
...
#             return self._to_actions_1d(priorities)
...
#         self._cached_n_tasks: int = -1
#         self._sorted_indices: list[int] = []
#         self._job_slices: list[tuple[int, int]] = []
#         self._sorted_indices_np: Any | None = None
#         self._sorted_indices_torch: Any | None = None

#     def _build_job_cache(self, obs: TabularRepresentation[ArrayLike]) -> None:
#         operation_ids = obs[self.operation_label]
#         jobs = obs["job_id"]
#         n_tasks = len(operation_ids)

#         sorted_indices = sorted(
#             range(n_tasks), key=lambda i: (int(jobs[i]), int(operation_ids[i]))
#         )

#         job_slices: list[tuple[int, int]] = []
#         start = 0
#         while start < n_tasks:
#             current_job = int(jobs[sorted_indices[start]])
#             end = start + 1
#             while (
#                 end < n_tasks and int(jobs[sorted_indices[end]]) == current_job
#             ):
#                 end += 1
#             job_slices.append((start, end))
#             start = end

#         self._cached_n_tasks = n_tasks
#         self._sorted_indices = sorted_indices
#         self._job_slices = job_slices

#         if NUMPY_AVAILABLE:
#             self._sorted_indices_np = np.asarray(sorted_indices, dtype=np.int64)

#         if TORCH_AVAILABLE:
#             self._sorted_indices_torch = None
...
#         if (
#             not self.memory
#             or self._cached_n_tasks != len(processing_times)
#             or not self._sorted_indices
#         ):
#             self._build_job_cache(obs)

#         if NUMPY_AVAILABLE and isinstance(processing_times, np.ndarray):
#             sorted_indices = self._sorted_indices_np
#             if sorted_indices is None:
#                 sorted_indices = np.asarray(
#                     self._sorted_indices, dtype=np.int64
#                 )

#             sorted_processing = processing_times[sorted_indices]
#             sorted_priorities = np.empty_like(sorted_processing, dtype=float)

#             for start, end in self._job_slices:
#                 # Reverse cumulative sum inside each job segment.
#                 sorted_priorities[start:end] = np.cumsum(
#                     sorted_processing[start:end][::-1]
#                 )[::-1]

#             priorities = np.empty_like(sorted_priorities)
#             priorities[sorted_indices] = sorted_priorities
#             return priorities

#         if TORCH_AVAILABLE and isinstance(processing_times, torch.Tensor):
#             if (
#                 self._sorted_indices_torch is None
#                 or self._sorted_indices_torch.device != processing_times.device
#             ):
#                 self._sorted_indices_torch = torch.tensor(
#                     self._sorted_indices,
#                     device=processing_times.device,
#                     dtype=torch.long,
#                 )

#             sorted_indices = self._sorted_indices_torch
#             sorted_processing = processing_times[sorted_indices]
#             sorted_priorities = torch.empty_like(sorted_processing)

#             for start, end in self._job_slices:
#                 sorted_priorities[start:end] = torch.cumsum(
#                     sorted_processing[start:end].flip(0), dim=0
#                 ).flip(0)

#             priorities = torch.empty_like(sorted_priorities)
#             priorities[sorted_indices] = sorted_priorities
#             return priorities

#         sorted_processing_list = [
#             float(processing_times[i]) for i in self._sorted_indices
#         ]
#         sorted_priorities_list = [0.0] * len(sorted_processing_list)

#         for start, end in self._job_slices:
#             cumulative = 0.0
#             for idx in range(end - 1, start - 1, -1):
#                 cumulative += sorted_processing_list[idx]
#                 sorted_priorities_list[idx] = cumulative

#         priorities_list = [0.0] * len(sorted_processing_list)
#         for sorted_idx, original_idx in enumerate(self._sorted_indices):
#             priorities_list[original_idx] = sorted_priorities_list[sorted_idx]

#         return initialize_array(priorities_list)
...
#         self._cached_n_tasks: int = -1
#         self._cached_priorities_list: list[float] = []
#         self._cached_priorities_np: Any | None = None
#         self._cached_priorities_torch: Any | None = None

#     def _build_priorities_cache(
#         self, obs: TabularRepresentation[ArrayLike]
#     ) -> None:
#         operations = obs[self.operation_label]
#         jobs = obs["job_id"]
#         n_tasks = len(operations)

#         sorted_indices = sorted(
#             range(n_tasks), key=lambda i: (int(jobs[i]), int(operations[i]))
#         )

#         sorted_priorities = [0.0] * n_tasks
#         start = 0
#         while start < n_tasks:
#             current_job = int(jobs[sorted_indices[start]])
#             end = start + 1
...
#                 sorted_priorities[idx] = float(remaining)
#                 remaining -= 1

#             start = end

#         priorities = [0.0] * n_tasks
#         for sorted_idx, original_idx in enumerate(sorted_indices):
#             priorities[original_idx] = sorted_priorities[sorted_idx]

#         self._cached_n_tasks = n_tasks
#         self._cached_priorities_list = priorities
#         self._cached_priorities_np = (
#             np.asarray(priorities, dtype=float) if NUMPY_AVAILABLE else None
#         )
#         self._cached_priorities_torch = None
...
#         if (
#             not self.memory
#             or self._cached_n_tasks != len(operations)
#             or not self._cached_priorities_list
#         ):
#             self._build_priorities_cache(obs)

#         if NUMPY_AVAILABLE and isinstance(operations, np.ndarray):
#             if self._cached_priorities_np is None:
#                 self._cached_priorities_np = np.asarray(
#                     self._cached_priorities_list, dtype=float
#                 )
#             priorities_np: ArrayLike = self._cached_priorities_np.copy()
#             return priorities_np

#         if TORCH_AVAILABLE and isinstance(operations, torch.Tensor):
#             if (
#                 self._cached_priorities_torch is None
#                 or self._cached_priorities_torch.device != operations.device
#             ):
#                 self._cached_priorities_torch = torch.tensor(
#                     self._cached_priorities_list,
#                     device=operations.device,
#                     dtype=operations.dtype,
#                 )

#             priorities_torch: ArrayLike = self._cached_priorities_torch.clone()
#             return priorities_torch

#         return initialize_array(self._cached_priorities_list)
