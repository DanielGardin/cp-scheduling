...
class Buffer:
    def __init__(
        ...
        self.buffer = TensorDict(
            {
                key: torch.empty(
                    (buffer_size, *shape), dtype=torch.float32, device=device
                )
    ...
    def add(self, **kwargs: Tensor) -> None:
        ...
        for key, value in kwargs.items():
            if value.requires_grad and not self.allow_grad:
                warn(
                    "Adding a tensor with grad to the buffer, but allow_grad is set to False. "
    ...
    def loader(
        self, batch_size: int, device: Device | None = None
    ) -> Iterable[TensorDict]:
