...
class End2EndActor(nn.Module):
    def __init__(
        self, d_model: int, n_heads: int = 1, n_layers: int = 1, dropout: float = 0.0
    ):
        super().__init__()
        ...
        self.job_encoder = nn.TransformerEncoder(
            transformer_encoder, n_layers, enable_nested_tensor=False
        )

        self.action_transformer = nn.TransformerEncoder(
            action_layer, n_layers, enable_nested_tensor=False
        )
    ...
    def forward(self, obs: Tensor) -> Tensor:
        ...
        encodings = self.action_transformer(
            encoded_jobs, src_key_padding_mask=finished_job, mask=job_resource_mask
        )
        ...
        mask = torch.logical_or(
            finished_job, torch.logical_and(input_mask[:, :, 2], obs[:, :, 1, 0].bool())
        )
