...
class PriorityDispatchingRule(ABC):
    ...
    def get_priority(
        self, obs: ObsType, current_time: Optional[int] = None
    ...
    def sample(
        ...
        if (temp is not None and temp <= 0.0) or (
...
class ShortestProcessingTime(PriorityDispatchingRule):
    ...
    def priority_rule(
        ...
        return [
            -processing_time for processing_time in tasks[self.processing_time_label]
        ]
...
class EarliestDueDate(PriorityDispatchingRule):
    ...
    def __init__(self, due_date_label: str = "due_date", job_info: bool = False):
    ...
    def priority_rule(
        ...
        due_dates = (
            ...
            if self.job_info
            else tasks[self.due_date_label]
        )
...
class WeightedShortestProcessingTime(PriorityDispatchingRule):
    ...
    def priority_rule(
        ...
        weights = (
            ...
            if self.job_info
            else tasks[self.weighted_label]
        )
...
class FirstInFirstOut(PriorityDispatchingRule):
    ...
    def __init__(
        self, release_time_label: str = "release_time", job_info: bool = False
    ):
        self.release_time_label = release_time_label
        self.job_info = job_info
    ...
    def priority_rule(
        ...
        return [float(time - release_date) for release_date in release_dates]
        ...
        return [
            (
                ...
                if due_date <= proc_time + time
                    ...
                    weight / proc_time * (T - due_date) / (T - time - proc_time)
                    if due_date < T
                    else 0
                )
            )
...
class ApparentTardinessCost(PriorityDispatchingRule):
    ...
    def priority_rule(
        ...
        return [
            weight
            / proc_time
            * exp(-max(0, due_date - time - proc_time) / (self.lookahead * T_mean))
            for weight, proc_time, due_date in zip(weights, processing_time, due_dates)
        ]
