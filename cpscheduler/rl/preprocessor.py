...
class TabularPreprocessor(nn.Module):
    ...
    def __init__(
        ...
        if self.remaining_action == "ignore":
            self.output_dim = len(numerical_indices) + len(categorical_indices) * categorical_embedding_dim
        
        elif self.remaining_action == "pass":
            if input_size is None:
                raise ValueError("input_size must be provided when remaining_action is 'pass'")

            self.output_dim = input_size + len(categorical_indices) * (categorical_embedding_dim - 1)
