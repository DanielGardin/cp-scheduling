...
def test_env(instance_name: str) -> None:
    ...
    assert new_reward == env.get_objective_values() - last_objective_values
    ...
    start_times, objective_value, is_optimal = env.get_cp_solution(timelimit=5)
    tasks = np.argsort(start_times)

    obs, reward, terminated, truncated, info = env.step(tasks, time_skip=None)
